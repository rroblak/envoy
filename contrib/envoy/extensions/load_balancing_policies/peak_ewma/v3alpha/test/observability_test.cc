...
protected:
  void SetUp() override {
    ...
    observability_ = std::make_unique<Observability>(
        *store_.rootScope(), time_source_, cost_, default_rtt_ms_);
  }
...
TEST_F(ObservabilityTest, CreateHostStats) {
  ...
  auto& cost_gauge = store_.gauge("peak_ewma.10.0.0.1:8080.cost", Stats::Gauge::ImportMode::NeverImport);
  auto& ewma_gauge = store_.gauge("peak_ewma.10.0.0.1:8080.ewma_rtt_ms", Stats::Gauge::ImportMode::NeverImport);
  auto& req_gauge = store_.gauge("peak_ewma.10.0.0.1:8080.active_requests", Stats::Gauge::ImportMode::NeverImport);
...
TEST_F(ObservabilityTest, MultipleHosts) {
  ...
  auto& host1_cost = store_.gauge("peak_ewma.10.0.0.1:8080.cost", Stats::Gauge::ImportMode::NeverImport);
  auto& host2_cost = store_.gauge("peak_ewma.10.0.0.2:8080.cost", Stats::Gauge::ImportMode::NeverImport);
  
  EXPECT_EQ(host1_cost.value(), 100);
  EXPECT_EQ(host2_cost.value(), 200);
  
  auto& host1_ewma = store_.gauge("peak_ewma.10.0.0.1:8080.ewma_rtt_ms", Stats::Gauge::ImportMode::NeverImport);
  auto& host2_ewma = store_.gauge("peak_ewma.10.0.0.2:8080.ewma_rtt_ms", Stats::Gauge::ImportMode::NeverImport);
...
TEST_F(ObservabilityTest, ConstructorParameters) {
  ...
  Observability custom_observability(
      *store_.rootScope(), time_source_, different_cost, different_default_rtt);
