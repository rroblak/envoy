...
size_t PeakEwmaLoadBalancer::calculateNewSampleCount(size_t last_processed, size_t current_write, size_t max_samples) {
...
double PeakEwmaLoadBalancer::calculateTimeBasedAlpha(uint64_t current_time_ns, uint64_t sample_time_ns) {
...
double PeakEwmaLoadBalancer::updateEwmaWithSample(double current_ewma, double new_rtt_ms, double alpha) {
  if (current_ewma == 0.0) {
...
void PeakEwmaLoadBalancer::processHostSamples(Upstream::HostConstSharedPtr /* host */, PeakEwmaHostLbPolicyData* data) {
  ...
  size_t num_new_samples = calculateNewSampleCount(last_processed, current_write, data->max_samples_);
