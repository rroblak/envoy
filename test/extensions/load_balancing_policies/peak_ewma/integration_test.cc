...
public:
  ...
  void runBasicLoadBalancing() {
    ...
    // Verify that P2C distributed requests across multiple upstreams
    EXPECT_GE(used_upstreams.size(), 2) << "Peak EWMA P2C should distribute across multiple upstreams";
  }

  void runLatencySensitiveRouting() {
    // Test that Peak EWMA prefers faster responding hosts
    
    // First, send requests to warm up the EWMA measurements
    for (int i = 0; i < 6; i++) {
      codec_client_ = makeHttpConnection(lookupPort("http"));
    ...
    // Verify that the slower upstream (1) receives fewer requests
    ...
    // The slow upstream should receive less than 50% of requests due to Peak EWMA's latency awareness
    EXPECT_LT(slow_upstream_ratio, 0.5) 
        << "Peak EWMA should reduce traffic to slower upstream. Slow upstream got " 
        << slow_upstream_requests << "/" << total_requests << " requests";
  }

  void runConfigValidation() {
    // Test that basic configuration is accepted and cluster starts successfully
    // This is validated by the fact that initializeConfig() completes without errors
    
    // Send a single request to verify the load balancer is functioning
    codec_client_ = makeHttpConnection(lookupPort("http"));
